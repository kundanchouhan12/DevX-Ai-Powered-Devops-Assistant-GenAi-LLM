...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        setStats(res.data);
        setHistory(res.data.recentScans || []);
      } catch (err) {
  ...
  useEffect(() => {
    ...
    setScanResult(null); // 🔥 clear old scan
    setStatus("queued");
    setProgress(0);
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      navigate(`/dashboard/${res.data.jobId}`); // 🔥 key fix
      setRepoUrl("");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6">
      {/* HEADER */}
      <h1 className="text-3xl font-bold mb-6">AI DevOps Dashboard 🚀</h1>
      ...
      <div className="flex gap-3 mb-6">
        ...
        <button
          ...
          disabled={loading}
          className="bg-blue-600 px-6 rounded font-semibold hover:bg-blue-700"
        >
      ...
      {scanResult && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <Stat label="Files" value={scanResult.files} />
          <Stat label="Lines" value={scanResult.lines} />
          <Stat label="Folders" value={scanResult.folders?.length || 0} />
          <Stat label="Tech Stack" value={Object.keys(scanResult.extensions || {}).length} />
        </div>
      )}
      ...
      {scanResult?.extensions && (
        <div className="mb-8">
          ...
          <div className="flex flex-wrap gap-2">
            {Object.entries(scanResult.extensions).map(([ext, count]) => (
              <span
                key={ext}
      ...
      {/* AI REPORT */}
      {scanResult?.aiReport && (
        <div className="bg-gray-900 p-5 rounded mb-8">
        ...
        <div>
          ...
          <div className="space-y-2">
            {history.map((item) => (
              <div
                key={item.jobId}
                onClick={() => navigate(`/dashboard/${item.jobId}`)}
                className="p-3 bg-gray-900 rounded cursor-pointer hover:bg-gray-800"
              >
                <p className="text-sm">{item.repoUrl}</p>
